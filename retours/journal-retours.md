...
| 2026-09-04 | Jacques (iPhone, puis portable) | **Aucun son sur les deux premières cartes** en entrant dans l'app. Ça ne démarre qu'à partir de la troisième. Sur le portable, cliquer explicitement sur le haut-parleur du mot débloque le reste de la séance. Se produit sur les **deux** plateformes. | **En cours de diagnostic.** `flipCard()` appelle bien `speakWord()` de façon synchrone dans le clic, donc ce n'est pas un blocage d'autoplay classique. Hypothèse principale : en « Mots au hasard », beaucoup de cartes viennent de B1-C1, qui n'ont **aucun fichier enregistré** — c'est la voix du navigateur qui parle, et `direAllemand()` appelle `speechSynthesis.cancel()` avant le premier `speak()`, ce qui laisse un moteur froid muet. En attente des lignes du journal audio pour confirmer. |
